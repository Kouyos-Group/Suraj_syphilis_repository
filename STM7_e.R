...
#RATIONALE: Same trasnmission rate of syphilis in HIV+ and HIV- MSM. 
...
##  SENSITIVITY ANALYSIS: Load the Rcpp program i.e., the mathematical model written in C++
sourceCpp("~/Downloads/STM_and_SRM/programs/STM7_e.cpp")
...
if(F){
  png("STM7_epop_plot_MSM.png", width = 14, height = 8, units = 'in', res = 100)
...
parameter_values_generator <- function(inc_factor, beta, risk_sort, overest_factor, theta){
  parameters <- c(inc_factor=inc_factor, beta=beta, risk_sort=risk_sort, 
...
random_initial_set_generator <- function(){
  ...
  parRange <- data.frame(min = c(1E-3, 1E-3, 1E-10, 1, 1, 0.5), max = c(30, 20, 0.1, 10, 10, 1))
  rownames(parRange) <- c("inc_factor", "beta", "theta", "pos_inc", "neg_inc", "risk_sort")
...
if(F){
  png("~/Downloads/STM_and_SRM/output/STM7_e_prop_occas.png", width = 8, height = 6, units = 'in', res = 100)
  ...
  png("~/Downloads/STM_and_SRM/output/STM7_e_syph_cases_plot.png", width = 8, height = 6, units = 'in', res = 100)
  par(mar=c(5, 12, 2, 1))
...
predictor_output_generator <- function(parameters, dataset){
  ...
  parms <- parameter_values_generator(inc_factor = parameters['inc_factor'], beta = parameters['beta'], risk_sort = parameters['risk_sort'], overest_factor = overest_factor_value, theta = parameters['theta'])
  ...
  ####
  ##  Required correction for sensitivity analysis
  predicted_dataset[, c('Diag_neg', 'Diag_low_neg', 'Diag_high_neg')] <- 
    predicted_dataset[, c('Diag_neg', 'Diag_low_neg', 'Diag_high_neg')]*parameters['inc_factor']
  
  predicted_dataset$Diag_all <- predicted_dataset$Diag_pos + predicted_dataset$Diag_neg
  ###
...
mod_function <- function(parameters){
  modFit(f=mod_cost, parameters, method = 'L-BFGS-B',
         lower = c(1E-3, 1E-3, 1E-10, 1E-3, 1E-3, 0.5), upper = c(30, 20, 0.1, 10, 10, 1),
...
save(data_16, file='STM7_e_optim_par_set.Rdata')

#####################      MODEL DIAGNOSIS OF FITTED MODEL   #######################################

####Run from here: Saved output
load('STM7_e_optim_par_set.Rdata')
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_e_sensFun.png', width = 12, height = 12, units = 'in', res = 150)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_e_collin.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
  par(mfcol=c(1,1))
...
upper_bound <- c(30, 20, 0.1, 10, 10, 1)
...
save(mc, file='STM7_e_optim_par_mc.Rdata')
...
save(summ_sR, file='STM7_e_optim_sensRange.Rdata')

#####################      MODEL DIAGNOSIS OF MCMC MODEL   #######################################

####Run from here: Saved output
load('STM7_e_optim_par_mc.Rdata')
load('STM7_e_optim_sensRange.Rdata')
...
if(F){
  ...
  png('~/Downloads/STM_and_SRM/output/STM7_e_prior_posterior_plot.png', width = 8, height = 8, units = 'in', res = 150)
  par(mar=c(5, 5, 2, 2))
  par(mfrow=c(3,2))
  for(i in 1:6){
...
#Plot of pairs
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_e_pairs_plot.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_e_parameter_space.png', width = 12, height = 8, units = 'in', res = 150)
  plot(mc)
  dev.off()
}
...
#Plot MCMC chain
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_e_cost_space.png', width = 12, height = 8, units = 'in', res = 150)
...
#Plot MCMC model fit
if(T){
  png(paste0("~/Downloads/STM_and_SRM/output/STM7_e_test_mcmc_occas.png"), width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
  par(mar=c(5, 18, 2, 1))
...
if(T){
  ...
  png("~/Downloads/STM_and_SRM/output/STM7_e_test_loglik_inc.png", width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=2,ncol=2, 1:4),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(2,2))
  par(mar=c(6, 20, 2, 1))
