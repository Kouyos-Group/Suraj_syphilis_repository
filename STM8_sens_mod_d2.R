...
#RATIONALE: Screening HIV+ MROP by 6 months and 3 months instead of 12 months
#           Transmission of syphilis from the non-infectious stage (10%)
...
sourceCpp("~/Downloads/STM_and_SRM/programs/STM7_d.cpp")
...
if(F){
  png("STM7_dpop_plot_MSM.png", width = 14, height = 8, units = 'in', res = 100)
...
if(F){
  ...
  png('STM7_d_sigma_kappa.png', width = 14, height = 8, units = 'in', res = 100)
  par(mfrow=c(1,2))
  par(mar=c(6, 8, 1, 1))
...
load('STM7_d2_optim_par_mc.Rdata')
load('STM7_d2_optim_sensRange.Rdata')

out <- predictor_output_generator(mc$bestpar)

#################     COUNTERFACTUAL SCENARIOS: 6 months   #######################################

parameter_values_generator <- function(beta_hiv, beta, risk_sort, overest_factor, theta){
  parameters <- c(beta_hiv=beta_hiv, beta=beta, risk_sort=risk_sort, 
                  overest_factor=overest_factor, theta=theta, kappa_neg=mean(risk$kappa),
                  deltaID_low_pos  = 16,  deltaLD_low_pos  = 1,    deltaIL_low_pos  = 12/3,  lambda_low_pos  = 12/1,
                  deltaID_high_pos = 15 + 1*2,  deltaLD_high_pos = 1,    deltaIL_high_pos = 12/3,  lambda_high_pos = 12/1,
                  deltaID_low_neg  = 16,  deltaLD_low_neg  = 0.5,  deltaIL_low_neg  = 12/3,  lambda_low_neg  = 12/1, 
                  deltaID_high_neg = 16,  deltaLD_high_neg = 0.5,  deltaIL_high_neg = 12/3,  lambda_high_neg = 12/1,
                  HR=3.167, sero_sort_pos=0.34, sero_sort_neg=0.91)
  return(parameters)
...
save(summ_sR1, file='STM8_sens_mod_d2_summ_sR1_6_months.Rdata')

#################     COUNTERFACTUAL SCENARIOS: 3 months   #######################################

parameter_values_generator <- function(beta_hiv, beta, risk_sort, overest_factor, theta){
  parameters <- c(beta_hiv=beta_hiv, beta=beta, risk_sort=risk_sort, 
                  overest_factor=overest_factor, theta=theta, kappa_neg=mean(risk$kappa),
                  deltaID_low_pos  = 16,  deltaLD_low_pos  = 1,    deltaIL_low_pos  = 12/3,  lambda_low_pos  = 12/1,
                  deltaID_high_pos = 15 + 1*4,  deltaLD_high_pos = 1,    deltaIL_high_pos = 12/3,  lambda_high_pos = 12/1,
                  deltaID_low_neg  = 16,  deltaLD_low_neg  = 0.5,  deltaIL_low_neg  = 12/3,  lambda_low_neg  = 12/1, 
                  deltaID_high_neg = 16,  deltaLD_high_neg = 0.5,  deltaIL_high_neg = 12/3,  lambda_high_neg = 12/1,
                  HR=3.167, sero_sort_pos=0.34, sero_sort_neg=0.91)
  return(parameters)
...
sR2 <- sensRange(predictor_output_generator, parInput = mc$pars, num=1000)
summ_sR2 <- (summary(sR2))
save(summ_sR2, file='STM8_sens_mod_d2_summ_sR2_3_months.Rdata')

#################     COUNTERFACTUAL SCENARIOS: PLOT   #######################################

load('STM8_sens_mod_d2_summ_sR1_6_months.Rdata')
load('STM8_sens_mod_d2_summ_sR2_3_months.Rdata')
...
col2rgb('grey') # 190 190 190
grey <- rgb(190, 190, 190, alpha = 100, maxColorValue = 255)

png("~/Downloads/STM_and_SRM/output/STM8_sens_mod_d2_occas_counter_test.png", width = 7, height = 6, units = 'in', res = 300)
layout(matrix(nrow=2,ncol=1, 1:4),widths=rep(6,6),heights=rep(6,6))
par(mfcol=c(2,1))
...
mtext('Incidence rate \n of syphilis \n in MSM with \n HIV diagnosis  \n(cases per 1000 py)',side=2,las=1, line=10, cex = 1.5, adj = 0.5)
...
polygon(c(out$year, rev(out$year)), c(summ_sR2[grep('inc_pos_per_1000', row.names(summ_sR2)),'q05'], rev(summ_sR2[grep('inc_pos_per_1000', row.names(summ_sR2)),'q95'])), col= grey, border = 'grey')
...
lines(out$year, summ_sR2[grep('inc_pos_per_1000', row.names(summ_sR2)),'q50'], xlab='Year', ylab='', cex.lab =1.25, lwd=3, col='black', cex.axis =1.25, cex=1.5, las=1, lty=1)
...
polygon(c(out$year, rev(out$year)), c(summ_sR2[grep('inc_neg_per_1000', row.names(summ_sR2)),'q05'], rev(summ_sR2[grep('inc_neg_per_1000', row.names(summ_sR2)),'q95'])), col= grey, border = 'grey')
lines(out$year, summ_sR[grep('inc_neg_per_1000', row.names(summ_sR)),'q50'], xlab='Year', ylab='', cex.lab =1.25, lwd=3, col='blue', cex.axis =1.25, cex=1.5, las=1, lty=1)
...
lines(out$year, summ_sR2[grep('inc_neg_per_1000', row.names(summ_sR2)),'q50'], xlab='Year', ylab='', cex.lab =1.25, lwd=3, col='black', cex.axis =1.25, cex=1.5, las=1, lty=1)
dev.off()

png("~/Downloads/STM_and_SRM/output/STM8_sens_mod_d2_occas_counter_test_legend.png", width = 10, height = 1.5, units = 'in', res = 300)
par(mar=c(0, 0, 0, 0))
plot(NULL, xlim=c(0,1), ylim=c(0,1), xlab='', ylab='', axes = F)
legend('top',c('Base-model (annual screening in MSM with HIV diagnosis and with nsP)', 'Screening MSM with HIV diagnosis and with nsP every 6 months', 'Screening MSM with HIV diagnosis and with nsP every 3 months'), 
       col=c('blue', 'red', 'black'), lty=c(1, 1,  1), lwd = c(3, 3, 3), cex = 1.5, bty='n')
dev.off()
...
inc_comp2 <- (summ_sR2[grep('Diag_all', row.names(summ_sR2)),]/summ_sR2[grep('total_denom', row.names(summ_sR2)),])['q50']

inc_comp <- inc_comp[nrow(inc_comp),]*1000; inc_comp
inc_comp1 <- inc_comp1[nrow(inc_comp1),]*1000; inc_comp1
inc_comp2 <- inc_comp2[nrow(inc_comp2),]*1000; inc_comp2

###Incidence reduction
(inc_comp - inc_comp1)/inc_comp
(inc_comp - inc_comp2)/inc_comp
