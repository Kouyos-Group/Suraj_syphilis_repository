...
#RATIONALE: Changing overestimation factor to 75% 
#(25% lower propotion than what we observe)
...
if(F){
  png("STM7_g1pop_plot_MSM.png", width = 14, height = 8, units = 'in', res = 100)
...
if(F){
  png("~/Downloads/STM_and_SRM/output/STM7_g1_prop_occas.png", width = 8, height = 6, units = 'in', res = 100)
  ...
  png("~/Downloads/STM_and_SRM/output/STM7_g1_syph_cases_plot.png", width = 8, height = 6, units = 'in', res = 100)
  par(mar=c(5, 12, 2, 1))
...
if(F){
  ...
  png('STM7_g1_sigma_kappa.png', width = 14, height = 8, units = 'in', res = 100)
  par(mfrow=c(1,2))
  par(mar=c(6, 8, 1, 1))
...
save(data_16, file='STM7_g1_optim_par_set.Rdata')

#####################      MODEL DIAGNOSIS OF FITTED MODEL   #######################################

####Run from here: Saved output
load('STM7_g1_optim_par_set.Rdata')
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_g1_sensFun.png', width = 12, height = 12, units = 'in', res = 150)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_g1_collin.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
  par(mfcol=c(1,1))
...
save(mc, file='STM7_g1_optim_par_mc.Rdata')
...
save(summ_sR, file='STM7_g1_optim_sensRange.Rdata')

#####################      MODEL DIAGNOSIS OF MCMC MODEL   #######################################

####Run from here: Saved output
load('STM7_g1_optim_par_mc.Rdata')
load('STM7_g1_optim_sensRange.Rdata')
...
if(F){
  ...
  png('~/Downloads/STM_and_SRM/output/STM7_g1_prior_posterior_plot.png', width = 8, height = 8, units = 'in', res = 150)
  par(mar=c(5, 5, 2, 2))
  par(mfrow=c(3,2))
  for(i in 1:6){
...
#Plot of pairs
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_g1_pairs_plot.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_g1_parameter_space.png', width = 12, height = 8, units = 'in', res = 150)
  plot(mc)
  dev.off()
}
...
#Plot MCMC chain
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_g1_cost_space.png', width = 12, height = 8, units = 'in', res = 150)
...
#Plot MCMC model fit
if(T){
  png(paste0("~/Downloads/STM_and_SRM/output/STM7_g1_test_mcmc_occas.png"), width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
  par(mar=c(5, 18, 2, 1))
...
if(T){
  ...
  png("~/Downloads/STM_and_SRM/output/STM7_g1_test_loglik_inc.png", width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=2,ncol=2, 1:4),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(2,2))
  par(mar=c(6, 20, 2, 1))
