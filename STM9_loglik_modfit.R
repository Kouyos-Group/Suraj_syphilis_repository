########################################################################################################
################################## loglikelihood vs ModFit ####################################
########################################################################################################

#AUTHOR: Suraj Balakrishna

#RATIONALE: To compare loglikelihood vs modfit
...
if(F){
  png("STM9_bpop_plot_MSM.png", width = 14, height = 8, units = 'in', res = 100)
...
if(F){
  ...
  png('STM9_b_sigma_kappa.png', width = 14, height = 8, units = 'in', res = 100)
  par(mfrow=c(1,2))
  par(mar=c(6, 8, 1, 1))
...
#Function to fit the model using loglikelihood
...
load('STM9_b_optim_par_mc.Rdata')
load('STM9_b_optim_sensRange.Rdata')
mc_ll <- mc
summ_sR_ll <- summ_sR
rm(mc); rm(summ_sR)

out <- predictor_output_generator(mc_ll$bestpar)
...
png("~/Downloads/STM_and_SRM/output/STM9_loglik_modfit.png", width = 12, height = 8, units = 'in', res = 100)
layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
par(mfcol=c(3,2))
par(mar=c(5, 18, 1, 1))

plot(out$year, inc$diag_first_low, xlab='Year', ylab='', type='p', ylim=c(0, 200), cex.lab =2, lwd=3, col='red', cex.axis=2, cex=1.5, las=1)
mtext('First episode of syphilis\nin MSM with HIV\ndiagnosis in the SHCS\nwithout non-steady\npartners',side=2,las=1, line=11, cex = 1, adj = 0.5)
lines(out$year, summ_sR_mod[grep('Diag_first_low', row.names(summ_sR_mod)),'q50'])
lines(out$year, summ_sR_ll[grep('Diag_first_low', row.names(summ_sR_ll)),'q50'], col='blue', lty=3)

plot(out$year, inc$diag_first_high, xlab='Year', ylab='', type='p', ylim=c(0, 200), cex.lab =2, lwd=3, col='red', cex.axis=2, cex=1.5, las=1)
mtext('First episode of syphilis\nin MSM with HIV\ndiagnosis in the SHCS\nwith non-steady\npartners',side=2,las=1, line=11, cex = 1, adj = 0.5)
lines(out$year, summ_sR_mod[grep('Diag_first_high', row.names(summ_sR_mod)),'q50'])
lines(out$year, summ_sR_ll[grep('Diag_first_high', row.names(summ_sR_ll)),'q50'], col='blue', lty=3)

plot(out$year, inc$diag_second, xlab='Year', ylab='', type='p', ylim=c(0, 200), cex.lab =2, lwd=3, col='red', cex.axis=2, cex=1.5, las=1)
mtext('Subsequent episode(s)\nin MSM with HIV\ndiagnosis in the SHCS',side=2,las=1, line=11, cex = 1, adj = 0.5)
lines(out$year, summ_sR_mod[grep('Diag_second([[:digit:]]+)', row.names(summ_sR_mod)),'q50'])
lines(out$year, summ_sR_ll[grep('Diag_second([[:digit:]]+)', row.names(summ_sR_ll)),'q50'], col='blue', lty=3)

plot(out$year, inc$all, xlab='Year', ylab='', type='p', ylim=c(0, 8000), cex.lab =2, lwd=3, col='red', cex.axis=2, cex=1.5, las=1)
mtext('MSM with HIV\ndiagnosis in the\nSHCS',side=2,las=1, line=11, cex = 1, adj = 0.5)
lines(out$year, summ_sR_mod[grep('SHCS', row.names(summ_sR_mod)),'q50'])
lines(out$year, summ_sR_ll[grep('SHCS', row.names(summ_sR_ll)),'q50'], col='blue', lty=3)

par(mar=c(0, 16, 0, 0))
plot(NULL, xlim=c(0,1), ylim=c(0,1), xlab='', ylab='', axes = F)
legend('center',c('Observed data', 'Model fit by minimizing SSR', 'Model fit by maximization of likelihood'), col=c('red', 'black', 'blue'), lty=c(NA, 1, 3), pch=c(1, NA, NA), lwd = c(3, 3, 3), cex = 2.25)

par(mar=c(5, 18, 1, 1))
plot(out$year, inc$diag_all, xlab='Year', ylab='', type='p', ylim=c(0, 500), cex.lab =2, lwd=3, col='red', cex.axis=2, cex=1.5, las=1)
mtext('Confirmed cases of\nsyphilis among MSM\nin Switzerland',side=2,las=1, line=10, cex = 1, adj = 0.5)
lines(out$year, summ_sR_mod[grep('Diag_all', row.names(summ_sR_mod)),'q50'])
lines(out$year, summ_sR_ll[grep('Diag_all', row.names(summ_sR_ll)),'q50'], col='blue', lty=3)
