...
#RATIONALE: Sensitivity analysis: nsCAI instead of nsP
...
risk_criteria <- 'p_occas_cuse'

## Load required data
inc <- read.csv('STM2_c_inc_data_new.csv')
inc <- inc[inc$year > 2005,]

inc_neg <- read.csv('STM2_f_inc_data_neg.csv')

risk_neg <- read.csv('STM2_f_french_doc.csv')

pop_data <- read.csv("~/Downloads/STM_and_SRM/Literature/BAG_pop/pop_data.csv")
pop_data <- pop_data[pop_data$Year %in% c(2006:2018),]

risk <- read.csv('STM2_e_survival_p_occas_cuse_sigma.csv')
risk2 <- read.csv('STM2_e_survival_p_occas_cuse_kappa.csv')
...
if(F){
  png("STM3_c_testpop_plot_MSM.png", width = 14, height = 8, units = 'in', res = 100)
...
parameter_values_generator <- function(beta_hiv, beta, risk_sort, overest_factor, theta){
  parameters <- c(beta_hiv=beta_hiv, beta=beta, risk_sort=risk_sort, 
                  ...
                  HR=3.364, sero_sort_pos=0.34, sero_sort_neg=0.91)
  return(parameters)
} 
...
if(F){
  png("~/Downloads/STM_and_SRM/output/STM3_c_test_prop_occas.png", width = 8, height = 6, units = 'in', res = 100)
  ...
  png("~/Downloads/STM_and_SRM/output/STM3_c_test_syph_cases_plot.png", width = 8, height = 6, units = 'in', res = 100)
  par(mar=c(5, 12, 2, 1))
...
if(F){
  ...
  png('STM3_c_test_sigma_kappa.png', width = 14, height = 8, units = 'in', res = 100)
  par(mfrow=c(1,2))
  par(mar=c(6, 8, 1, 1))
...
save(data_16, file='STM3_c_test_optim_par_set.Rdata')

#####################      MODEL DIAGNOSIS OF FITTED MODEL   #######################################

####Run from here: Saved output
load('STM3_c_test_optim_par_set.Rdata')
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_sensFun.png', width = 12, height = 12, units = 'in', res = 150)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_collin.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
  par(mfcol=c(1,1))
...
save(mc, file='STM3_c_test_optim_par_mc.Rdata')
...
save(summ_sR, file='STM3_c_test_optim_sensRange.Rdata')

#####################      MODEL DIAGNOSIS OF MCMC MODEL   #######################################

####Run from here: Saved output
load('STM3_c_test_optim_par_mc.Rdata')
load('STM3_c_test_optim_sensRange.Rdata')
...
if(F){
  ...
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_prior_posterior_plot.png', width = 8, height = 8, units = 'in', res = 150)
  par(mar=c(5, 5, 2, 2))
  par(mfrow=c(3,2))
  for(i in 1:6){
...
#Plot of pairs
if(F){
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_pairs_plot.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_parameter_space.png', width = 12, height = 8, units = 'in', res = 150)
  plot(mc)
  dev.off()
}
...
#Plot MCMC chain
if(F){
  png('~/Downloads/STM_and_SRM/output/STM3_c_test_cost_space.png', width = 12, height = 8, units = 'in', res = 150)
...
#Plot MCMC model fit
if(T){
  png(paste0("~/Downloads/STM_and_SRM/output/STM3_c_test_test_mcmc_occas.png"), width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
  par(mar=c(5, 18, 2, 1))
  ...
  par(mar=c(5, 18, 2, 1))
  plot(out$year, inc$diag_all, xlab='Year', ylab='', type='p', ylim=c(0, 550), cex.lab =2, lwd=1, col='red', cex.axis=2, cex=1, las=1)
...
if(T){
  ...
  png("~/Downloads/STM_and_SRM/output/STM3_c_test_test_loglik_inc.png", width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=2,ncol=2, 1:4),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(2,2))
  par(mar=c(6, 20, 2, 1))
