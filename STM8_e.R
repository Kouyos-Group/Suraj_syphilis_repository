...
#RATIONALE: Screening HIV- MROP by 6 months and 3 months instead of 2 years
...
parameter_values_generator <- function(beta_hiv, beta, risk_sort, overest_factor, theta){
  parameters <- c(beta_hiv=beta_hiv, beta=beta, risk_sort=risk_sort, 
                  ...
                  deltaID_high_pos = 16,  deltaLD_high_pos = 1,    deltaIL_high_pos = 12/3,  lambda_high_pos = 12/1,
                  deltaID_low_neg  = 16,  deltaLD_low_neg  = 0.5,  deltaIL_low_neg  = 12/3,  lambda_low_neg  = 12/1, 
                  deltaID_high_neg = 15.5 + 0.5*4,  deltaLD_high_neg = 0.5,  deltaIL_high_neg = 12/3,  lambda_high_neg = 12/1,
                  HR=3.167, sero_sort_pos=0.34, sero_sort_neg=0.91)
  return(parameters)
} 
...
save(summ_sR1, file='STM8_e_summ_sR1_6_months.Rdata')
...
parameter_values_generator <- function(beta_hiv, beta, risk_sort, overest_factor, theta){
  parameters <- c(beta_hiv=beta_hiv, beta=beta, risk_sort=risk_sort, 
                  ...
                  deltaID_high_pos = 16,  deltaLD_high_pos = 1,    deltaIL_high_pos = 12/3,  lambda_high_pos = 12/1,
                  deltaID_low_neg  = 16,  deltaLD_low_neg  = 0.5,  deltaIL_low_neg  = 12/3,  lambda_low_neg  = 12/1, 
                  deltaID_high_neg = 15.5 + 0.5*8,  deltaLD_high_neg = 0.5,  deltaIL_high_neg = 12/3,  lambda_high_neg = 12/1,
                  HR=3.167, sero_sort_pos=0.34, sero_sort_neg=0.91)
  return(parameters)
} 
...
save(summ_sR2, file='STM8_e_summ_sR2_3_months.Rdata')

#################     COUNTERFACTUAL SCENARIOS: PLOT   #######################################

load('STM8_e_summ_sR1_6_months.Rdata')
load('STM8_e_summ_sR2_3_months.Rdata')
...
png("~/Downloads/STM_and_SRM/output/STM8_e_occas_counter_test.png", width = 7, height = 6, units = 'in', res = 300)
...
png("~/Downloads/STM_and_SRM/output/STM8_e_occas_counter_test_legend.png", width = 10, height = 1.5, units = 'in', res = 300)
par(mar=c(0, 0, 0, 0))
plot(NULL, xlim=c(0,1), ylim=c(0,1), xlab='', ylab='', axes = F)
legend('top',c('Base-model (Screening MSM without HIV diagnosis and with nsP every 2 years)', 'Screening MSM without HIV diagnosis and with nsP every 6 months', 'Screening MSM without HIV diagnosis and with nsP every 3 months'), 
