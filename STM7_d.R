...
#RATIONALE: Transmission of syphilis from the non-infectious stage (1%)
...
save(data_16, file='STM7_d_optim_par_set.Rdata')

#####################      MODEL DIAGNOSIS OF FITTED MODEL   #######################################

####Run from here: Saved output
load('STM7_d_optim_par_set.Rdata')
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_d_sensFun.png', width = 12, height = 12, units = 'in', res = 150)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_d_collin.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
  par(mfcol=c(1,1))
...
save(mc, file='STM7_d_optim_par_mc.Rdata')
...
save(summ_sR, file='STM7_d_optim_sensRange.Rdata')
...
if(F){
  ...
  png('~/Downloads/STM_and_SRM/output/STM7_d_prior_posterior_plot.png', width = 8, height = 8, units = 'in', res = 150)
  par(mar=c(5, 5, 2, 2))
  par(mfrow=c(3,2))
  for(i in 1:6){
...
#Plot of pairs
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_d_pairs_plot.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_d_parameter_space.png', width = 12, height = 8, units = 'in', res = 150)
  plot(mc)
  dev.off()
}
...
#Plot MCMC chain
if(F){
  png('~/Downloads/STM_and_SRM/output/STM7_d_cost_space.png', width = 12, height = 8, units = 'in', res = 150)
...
#Plot MCMC model fit
if(T){
  png(paste0("~/Downloads/STM_and_SRM/output/STM7_d_test_mcmc_occas.png"), width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
  par(mar=c(5, 18, 2, 1))
...
if(T){
  ...
  png("~/Downloads/STM_and_SRM/output/STM7_d_test_loglik_inc.png", width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=2,ncol=2, 1:4),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(2,2))
  par(mar=c(6, 20, 2, 1))
