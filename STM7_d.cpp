...
// Rationale: Individuals in non-infectious stage contribute for the force of infection.
...
NumericMatrix bmv_modelQSS(int mult,
                           ...
                           double beta_hiv, double beta, double risk_sort, 
                           ...
                           NumericVector birth, NumericVector death, NumericVector migration, double trans_factor){
  ...
  for (int i = 1; i <= num_years*mult; i += 1){
    ...
    double inf_low_pos  = variables(1,i-1)  + variables(5,i-1)  + trans_factor*(variables(2,i-1)  + variables(6,i-1));
    double inf_high_pos = variables(9,i-1)  + variables(13,i-1) + trans_factor*(variables(10,i-1) + variables(14,i-1));
    double inf_low_neg  = variables(17,i-1) + variables(21,i-1) + trans_factor*(variables(18,i-1) + variables(22,i-1));
    double inf_high_neg = variables(25,i-1) + variables(29,i-1) + trans_factor*(variables(26,i-1) + variables(30,i-1));
    
    
    double   force_on_low_pos_by_low_pos   = beta_hiv*   ((  sero_sort_pos)*(  risk_sort)*inf_low_pos/sum_low_pos)  ;
    double   force_on_low_pos_by_high_pos  = beta_hiv*HR*((  sero_sort_pos)*(1-risk_sort)*inf_high_pos/sum_high_pos);
    double   force_on_low_pos_by_low_neg   = beta_hiv*   ((1-sero_sort_pos)*(  risk_sort)*inf_low_neg/sum_low_neg)  ;
    double   force_on_low_pos_by_high_neg  = beta_hiv*HR*((1-sero_sort_pos)*(1-risk_sort)*inf_high_neg/sum_high_neg);
    
    double   force_on_high_pos_by_low_pos  = beta_hiv*HR*((  sero_sort_pos)*(1-risk_sort)*inf_low_pos/sum_low_pos)  ;
    double   force_on_high_pos_by_high_pos = beta_hiv*HR*((  sero_sort_pos)*(  risk_sort)*inf_high_pos/sum_high_pos);
    double   force_on_high_pos_by_low_neg  = beta_hiv*HR*((1-sero_sort_pos)*(1-risk_sort)*inf_low_neg/sum_low_neg)  ;
    double   force_on_high_pos_by_high_neg = beta_hiv*HR*((1-sero_sort_pos)*(  risk_sort)*inf_high_neg/sum_high_neg);
