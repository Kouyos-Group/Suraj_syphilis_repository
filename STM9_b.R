...
#RATIONALE: loglikelihood method
...
mod_function <- function(parameters){
  modFit(f=loglik, parameters, method = 'L-BFGS-B',
...
save(data_16, file='STM9_b_optim_par_set.Rdata')

#####################      MODEL DIAGNOSIS OF FITTED MODEL   #######################################

####Run from here: Saved output
load('STM9_b_optim_par_set.Rdata')
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM9_b_sensFun.png', width = 12, height = 12, units = 'in', res = 150)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM9_b_collin.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
  par(mfcol=c(1,1))
...
mc <- modMCMC(f=loglik, p = data_16$par,  
              updatecov = 10 , 
              ntrydr = 2,
              jump = jump_value,
              lower = lower_bound, upper = upper_bound,
              niter = 12000, burninlength = 2000)

save(mc, file='STM9_b_optim_par_mc.Rdata')
...
save(summ_sR, file='STM9_b_optim_sensRange.Rdata')
...
if(F){
  ...
  png('~/Downloads/STM_and_SRM/output/STM9_b_prior_posterior_plot.png', width = 8, height = 8, units = 'in', res = 150)
  par(mar=c(5, 5, 2, 2))
  par(mfrow=c(3,2))
  for(i in 1:6){
...
#Plot of pairs
if(F){
  png('~/Downloads/STM_and_SRM/output/STM9_b_pairs_plot.png', width = 12, height = 12, units = 'in', res = 150)
  par(mar=c(5, 5, 5, 5))
...
if(F){
  png('~/Downloads/STM_and_SRM/output/STM9_b_parameter_space.png', width = 12, height = 8, units = 'in', res = 150)
  plot(mc)
  dev.off()
}
...
#Plot MCMC chain
if(F){
  png('~/Downloads/STM_and_SRM/output/STM9_b_cost_space.png', width = 12, height = 8, units = 'in', res = 150)
...
#Plot MCMC model fit
if(T){
  png(paste0("~/Downloads/STM_and_SRM/output/STM9_b_test_mcmc_occas.png"), width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=3,ncol=2, 1:6),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(3,2))
  par(mar=c(5, 18, 2, 1))
...
if(T){
  ...
  png("~/Downloads/STM_and_SRM/output/STM9_b_test_loglik_inc.png", width = 12, height = 8, units = 'in', res = 300)
  layout(matrix(nrow=2,ncol=2, 1:4),widths=rep(6,6),heights=rep(6,6))
  par(mfcol=c(2,2))
  par(mar=c(6, 20, 2, 1))
...
out$Diag_all
inc_comp <- out$Diag_all*1000/out$total_denom
inc_comp
summary(inc_comp)

#effect of HIV+
pos_inc_comp <- out$Diag_pos/(out$denom_pos_high + out$denom_pos_low)
neg_inc_comp <- out$Diag_neg/(out$denom_neg_high + out$denom_neg_low)
summary(pos_inc_comp/neg_inc_comp)

#effect of risk behaviour
high_inc_comp_pos <- out$Diag_high_pos/out$denom_pos_high
low_inc_comp_pos <- out$Diag_low_pos/out$denom_pos_low
high_inc_comp_neg <- out$Diag_high_neg/out$denom_neg_high
low_inc_comp_neg <- out$Diag_low_neg/out$denom_neg_low
colMeans(rbind(summary(high_inc_comp_neg/low_inc_comp_neg), summary(high_inc_comp_pos/low_inc_comp_pos)))
